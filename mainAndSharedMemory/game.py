...
def ecoute(client_socket, address, jeu,n_player):
    with client_socket:
        global play
        print("Connected to client:", address)
        create_shared_memory(client_socket, n_player)
        with num.get_lock():
...
if __name__ == "__main__":
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket_px:
        ...
        while True:
            ...
            p = Process(target=ecoute, args=(client_socket_px, address_px, jeu,n_player,))
            p.start()
